...
/// True when the user has authored their own ambient declaration
/// next to a `.svelte` source. Two shapes are recognised:
///
/// - `Foo.svelte.d.ts` — TS's standard ambient-for-extension shape
///   (matches `import './Foo.svelte'` directly when present in the
///   resolution roots).
/// - `Foo.d.svelte.ts` — the `allowArbitraryExtensions` ambient form
///   that TS resolves for non-`.ts` extensions; mirrors the path
///   shape our cache writes.
///
/// When either exists, our cache MUST NOT write its own re-export
/// ambient at the mirrored path — under `rootDirs` the cache is
/// listed first and would shadow the user's. The user's ambient
/// declares the surface (e.g. `export declare const a: boolean;`)
/// that consumer modules import; our re-export-only ambient only
/// surfaces `default` from the overlay, so any named import fires
/// TS2614 "no exported member" against the shadowed view.
fn user_has_svelte_ambient(svelte_source: &Path) -> bool {
    let parent = match svelte_source.parent() {
        Some(p) => p,
        None => return false,
    };
    let file_name = match svelte_source.file_name().and_then(|s| s.to_str()) {
        Some(n) => n,
        None => return false,
    };
    // `Foo.svelte` → bare = `Foo`.
    let bare = match file_name.strip_suffix(".svelte") {
        Some(b) => b,
        None => return false,
    };
    parent.join(format!("{bare}.svelte.d.ts")).is_file()
        || parent.join(format!("{bare}.d.svelte.ts")).is_file()
}
